...
_LOGGER = logging.getLogger(log_files.ta_frmk_conf)
...
def parse_conf_xml_dom(xml_content):
    ...
    for entry in xml_conf.iterfind(entry_path):
        ...
        for key in entry.iterfind(key_path):
            if key.get("name") not in ("eai:acl", "eai:attributes"):
                stanza_obj[key.get("name")] = key.text
...
def reload_confs(confs, session_key, splunkd_uri="https://localhost:8089",
    ...
    for conf in new_confs:
        ...
        print "session_key",session_key,"endpoint:",endpoint, "resp:",resp,"content:",content
...
class ConfManager(object):
    ...
    def update_conf_properties(self, user, appname, file_name,
                               stanza, key_values):
        """
        @return: True if update is successful, otherwise return False,wjo
        """
    ...
    def get_conf(self, user, appname, file_name, stanza=None):
        ...
        print 'uri:',uri,"msg",msg,"content",content
